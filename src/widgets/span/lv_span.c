...
void lv_span_set_text(lv_span_t * span, const char * text)
{
    ...
    text_alloc_len = _lv_text_ap_calc_bytes_count(text);
#else
...
#if LV_USE_ARABIC_PERSIAN_CHARS
    size_t text_alloc_len = _lv_text_ap_calc_bytes_count(text);
    span->txt = lv_malloc(text_alloc_len);
    LV_ASSERT_MALLOC(span->txt)
    _lv_text_ap_proc(text, span->txt);
    span->static_flag = 0;
...
static void lv_draw_span(lv_obj_t * obj, lv_layer_t * layer)
{
    ...
    if(base_dir == LV_BASE_DIR_AUTO) {
        base_dir = _lv_bidi_detect_base_dir(cur_txt) == LV_BASE_DIR_RTL ? LV_BASE_DIR_RTL : LV_BASE_DIR_AUTO;
    }
            ...
            if(ellipsis_valid) {
                ...
                if(base_dir == LV_BASE_DIR_RTL) {
                    if(txt_bytes > label_draw_dsc.text_length) {
                        ...
                        if(_lv_bidi_detect_base_dir(bidi_txt) == LV_BASE_DIR_RTL) {
