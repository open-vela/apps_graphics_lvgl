...
static void _flatten_path(const lv_vector_path_t * path, path_generate_cb cb, void * user_data)
{
    uint32_t pidx = 0;
    ...
    for(uint32_t i = 0; i < len; i++) {
        switch(op[i]) {
            case LV_VECTOR_PATH_OP_MOVE_TO: {
                    ...
                    cb(LV_VECTOR_PATH_OP_MOVE_TO, pt, user_data);
                    pidx += 1;
                }
                break;
            case LV_VECTOR_PATH_OP_LINE_TO: {
                    lv_fpoint_t * pt = lv_array_at(&path->points, pidx);
                    cb(LV_VECTOR_PATH_OP_LINE_TO, pt, user_data);
                    pidx += 1;
                }
                break;
            case LV_VECTOR_PATH_OP_QUAD_TO: {
                    ...
                    _flatten_quadratic_curve(last_pt, pt1, pt2, cb, user_data);
                    pidx += 2;
                }
                break;
            case LV_VECTOR_PATH_OP_CUBIC_TO: {
                    ...
                    _flatten_cubic_curve(last_pt, pt1, pt2, pt3, cb, user_data);
                    pidx += 3;
                }
                break;
            case LV_VECTOR_PATH_OP_CLOSE: {
                    cb(LV_VECTOR_PATH_OP_CLOSE, NULL, user_data);
                }
                break;
        }
    }

    cb(LV_VECTOR_POLYGON_STOP, NULL, user_data); // for polygon stop flag
}
...
bool lv_vector_path_flatten(const lv_vector_path_t * path, path_generate_cb cb, void * user_data)
{
    ...
    _flatten_path(path, cb, user_data);
    return true;
}
...
bool lv_vector_path_to_polygon(lv_vector_path_t * result, const lv_vector_path_t * path)
{
    ...
    if(path->flags & PATH_FLAG_POLYGON) {
        ...
        bool ret = lv_vector_path_flatten(path, path_to_polygon_cb, result);
        if(ret) {
