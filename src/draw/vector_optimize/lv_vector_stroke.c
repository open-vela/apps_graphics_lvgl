 ...
 *   GLOBAL FUNCTIONS
 **********************/
bool lv_vector_stroke_generate(const lv_platform_path_base_t * impl, const lv_vector_stroke_dsc_t * stroke_dsc,
                               path_generate_cb cb, void * user_data)
{
    ...
    if(stroke_dsc->dash_count > 0) { /* dash line */
        ...
        ret = lv_vector_path_flatten(impl, path_to_stroke_cb, &line_gen);
    }
                              ...
                              const lv_vector_stroke_dsc_t * stroke_dsc)
{
    ...
    ret = lv_vector_stroke_generate(path->impl, stroke_dsc, polygon_to_path, result);
