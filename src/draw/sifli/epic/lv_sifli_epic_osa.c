 ...
 *********************/
    ...
    #define LV_SIFLI_EPIC_NS_PER_SEC 1000000000L
#endif
...
#if defined(__NuttX__)
    static void _epic_timespec_add_us(struct timespec * ts, uint32_t us);
#endif
...
lv_result_t lv_epic_osa_thread_sync_wait(void)
{
    ...
    _epic_timespec_add_us(&timeout, LV_SIFLI_EPIC_WAIT_TIMEOUT_US);

    do {
        ret = sem_timedwait(&epic_sem, &timeout);
...
static void _epic_timespec_add_us(struct timespec * ts, uint32_t us)
{
    ...
    if(ts->tv_nsec >= LV_SIFLI_EPIC_NS_PER_SEC) {
        ts->tv_sec++;
        ts->tv_nsec -= LV_SIFLI_EPIC_NS_PER_SEC;
    }
}
#endif

#endif /*LV_USE_DRAW_SIFLI_EPIC*/
