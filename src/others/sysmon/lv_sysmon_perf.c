...
#define perf_global LV_GLOBAL_DEFAULT()->sysmon_perf.backend
...
static void lv_sysmon_perf_check_display(void);
static void lv_sysmon_perf_update_global(lv_event_code_t code);
static void * lv_sysmon_perf_circle_buf_append(lv_circle_buf_t * circle_buf, void * data);
static void lv_sysmon_perf_init_info(lv_sysmon_perf_info_t * info);
static void lv_sysmon_perf_update_info(lv_sysmon_perf_info_t * info, lv_event_code_t code);
static void lv_sysmon_perf_calculate_info(lv_sysmon_perf_info_t * info);
...
{
    _lv_ll_init(&perf_global.instances_ll, sizeof(lv_sysmon_perf_t));
    lv_sysmon_perf_check_display();
}

lv_sysmon_perf_t * lv_sysmon_perf_create(const char * tag, size_t max_events, size_t max_scrolls)
{
    lv_sysmon_perf_t * perf = _lv_ll_ins_tail(&perf_global.instances_ll);
    if(!perf) {
        return NULL;
    }
...
void lv_sysmon_perf_destroy(lv_sysmon_perf_t * perf)
{
    ...
    _lv_ll_remove(&perf_global.instances_ll, perf);
    lv_free(perf);
}
...
void lv_sysmon_perf_reset_data(lv_sysmon_perf_t * perf, lv_sysmon_perf_type_t types)
{
    ...
    if(types & LV_SYSMON_PERF_TYPE_OVERALL) {
        lv_sysmon_perf_init_info(&perf->data.overall);
    }
...
const lv_sysmon_perf_data_t * lv_sysmon_perf_get_data(lv_sysmon_perf_t * perf)
{
    ...
    lv_sysmon_perf_calculate_info(&perf->data.overall);
    if(perf->current_scroll) {
        lv_sysmon_perf_calculate_info(perf->current_scroll);
    }
...
void lv_sysmon_perf_event(lv_event_t * e)
{
    ...
    lv_sysmon_perf_update_global(code);

    _LV_LL_READ(&perf_global.instances_ll, perf) {
        if(perf->running) {
            lv_sysmon_perf_update_info(&perf->data.overall, code);
...
static void lv_sysmon_perf_check_display(void)
{
    if(!perf_global.inited && lv_display_get_default()) {
        lv_display_add_event_cb(lv_display_get_default(), lv_sysmon_perf_event, LV_EVENT_ALL, NULL);
        perf_global.inited = true;
    }
}
...
static void lv_sysmon_perf_calculate_info(lv_sysmon_perf_info_t * info)
{
    ...
    info->calculated.fps = info->measured.refr_interval_sum ? (1000 * info->measured.refr_cnt / info->calculated.duration) :
                           0;
    ...
    switch(code) {
        case LV_EVENT_SCROLL_BEGIN:
            if(!perf->current_scroll) {
                ...
                lv_sysmon_perf_init_info(perf->current_scroll);
            }
            break;
        case LV_EVENT_SCROLL_END:
            if(perf->current_scroll) {
        ...
        default:
            if(perf->current_scroll) {
                lv_sysmon_perf_update_info(perf->current_scroll, code);
            }
            break;
    }
}
