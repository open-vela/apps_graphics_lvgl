...
lv_result_t lv_event_send(lv_event_list_t * list, lv_event_t * e, bool preprocess)
{
    ...
    if(!is_traversing && lv_event_has_flag(list, LV_EVENT_FLAG_REMOVED)) {
        LV_TRACE_EVENT("Delay delete begin,event list : %" PRIu32, size);
        ...
        for(i = 0; i < lv_array_size(&list->array);) {
            ...
            if(*dsc_null == NULL) {
                ...
                LV_TRACE_EVENT("Event list is compacted. index: %" PRIu32, i);
                continue;
            }
            ++i;
        }
        LV_TRACE_EVENT("Delay delate end,event list : %" PRIu32, i);
