...
static lv_obj_t * test_calender_active_screen = NULL;
static lv_obj_t * _test_calendar = NULL;

void setUp(void)
{
    test_calender_active_screen = lv_screen_active();
    _test_calendar = lv_calendar_create(test_calender_active_screen);
}

void tearDown(void)
{
    lv_obj_clean(test_calender_active_screen);
}

void test_calendar_creation_successful(void)
{
    TEST_ASSERT_NOT_NULL(_test_calendar);
}
...
void test_calendar_set_today_date(void)
{
    ...
    lv_calendar_set_today_date(_test_calendar, today.year, today.month, today.day);

    const lv_calendar_date_t * date_after_test = lv_calendar_get_today_date(_test_calendar);
...
void test_calendar_set_today_date_gui(void)
{
    ...
    lv_calendar_set_today_date(_test_calendar, today.year, today.month, today.day);
    lv_calendar_set_showed_date(_test_calendar, 2022, 9);
...
void test_calendar_set_highlighted_dates(void)
{
    ...
    lv_calendar_set_highlighted_dates(_test_calendar, highlighted_days, 3);

    const lv_calendar_date_t * highlighted_days_after_test = lv_calendar_get_highlighted_dates(_test_calendar);
...
void test_calendar_set_highlighted_dates_gui(void)
{
    ...
    lv_calendar_set_showed_date(_test_calendar, 2022, 2);
...
void test_calendar_set_day_names_gui(void)
{
    ...
    lv_calendar_set_day_names(_test_calendar, day_names);
...
void test_calendar_get_highlighted_dates_num(void)
{
    ...
    TEST_ASSERT_EQUAL_INT16(3, lv_calendar_get_highlighted_dates_num(_test_calendar));
}
...
void test_calendar_header_arrow_create_gui(void)
{
    lv_calendar_header_arrow_create(_test_calendar);

    lv_calendar_set_showed_date(_test_calendar, 2022, 10);    // Use October to avoid month name sliding
...
void test_calendar_event_key_down_gui(void)
{
    ...
    lv_obj_send_event(_test_calendar, LV_EVENT_KEY, (void *) &key);
...
void test_calendar_get_pressed_date_null(void)
{
    ...
    lv_result_t result = lv_calendar_get_pressed_date(_test_calendar, &pressed_date);
...
void test_calendar_get_btnmatrix(void)
{
    lv_obj_t * btnm = lv_calendar_get_btnmatrix(_test_calendar);
