...
{
    lv_obj_set_size(_test_slider, 200, 20);
    lv_obj_center(_test_slider);
    lv_obj_update_layout(_test_slider);

    TEST_ASSERT_FALSE(lv_slider_is_dragged(_test_slider));

    lv_area_t coords;
    lv_obj_get_coords(_test_slider, &coords);
    ...
    TEST_ASSERT_TRUE(lv_slider_is_dragged(_test_slider));
...
void test_slider_scroll_chain_horizontal(void)
{
    ...
    lv_test_mouse_release();
    lv_obj_send_event(_test_slider, LV_EVENT_RELEASED, NULL);

    /* Horizontal ptr should allow vertical scroll chain */
    TEST_ASSERT_TRUE(lv_obj_has_flag(_test_slider, LV_OBJ_FLAG_SCROLL_CHAIN_VER));
}

void test_slider_scroll_chain_vertical(void)
{
    lv_obj_set_size(_test_slider, 20, 200);
    lv_obj_update_layout(_test_slider);
    ...
    TEST_ASSERT_TRUE(lv_obj_has_flag(_test_slider, LV_OBJ_FLAG_SCROLL_CHAIN_HOR));
}
...
void test_slider_range_mode_key_decrement_left_value(void)
{
    ...
    lv_slider_set_left_value(sliderRangeMode, 20, LV_ANIM_OFF);
...
static void assert_slider_drag_start_selection(lv_obj_t * slider, int32_t x, int32_t y,
...
void test_slider_range_mode_drag_start_value_selection(void)
{
    ...
    lv_slider_set_left_value(sliderRangeMode, 20, LV_ANIM_OFF); /* Left knob at ~20% */
    lv_obj_update_layout(sliderRangeMode);
...
void test_slider_vertical_range_mode_drag_start_value_selection(void)
{
    ...
    lv_slider_set_left_value(slider_ver, 20, LV_ANIM_OFF);
